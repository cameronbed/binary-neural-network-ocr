...
void test_fsm(Vsystem_controller *dut)
{
    ...
    dut->spi_cs_n = 1;
    tick(dut, 3);
    std::cout << "[DEBUG] send_image_pin = " << int(dut->send_image_pin) << "\n";
    assert(dut->send_image_pin == 1 && "[TEST FSM] Expected send_image_pin = 1");
    ...
    std::cout << "[DEBUG] send_image_pin = " << int(dut->send_image_pin) << "\n";
    assert(dut->send_image_pin == 1 && "[TEST FSM] send_image_pin should remain high during image RX");

    // ---------- [5] send_image_pin SHOULD RESET ----------
    tick(dut, 10); // Allow FSM to return to IDLE
    assert(dut->send_image_pin == 0 && "[TEST FSM] send_image_pin not cleared after return to S_IDLE");
    ...
    dut->spi_cs_n = 1;
    tick(dut, 5);
    assert(dut->status_ready_pin == 0 && "[TEST FSM] Unexpected status_ready_pin for unknown cmd");
    assert(dut->send_image_pin == 0 && "[TEST FSM] Unexpected send_image_pin for unknown cmd");
    ...
    assert(dut->status_ready_pin == 0);
    assert(dut->send_image_pin == 0);
